...
{#if kombucha}
	...
	<div class={css({ p: 4 })}>
		...
		{#if !data.isLoggedIn}
			<div class={css({ color: 'textMuted' })}>
				<a href="/login">Log in</a> or <a href="/register">register</a> to add a review
			</div>
		{/if}
		...
		{#if kombucha.reviews && hasReviews}
			<Reviews reviews={kombucha.reviews} />
		{:else}
			<p class={css({ color: 'textMuted' })}>No reviews yet</p>
		{/if}
	</div>
{/if}
